...
from openalea.cnmetabolism import simulation as cnmetabolism_simulation, model as cnmetabolism_model, parameters as cnmetabolism_parameters
from openalea.respiration import model as respiration_model
from openalea.integration import tools as integration_tools

"""
    cnmetabolism.postprocessing
    ~~~~~~~~~~~~~~~~~~~~~~

    The module :mod:`cnmetabolism.postprocessing` defines post-processing to apply
    on CN-Metabolism outputs, and provides a front-end to automatize the generation of graphs
...
#: the time index
T_INDEX = cnmetabolism_simulation.Simulation.T_INDEX

#: the index to locate the plants in the modelled system
PLANTS_INDEXES = cnmetabolism_simulation.Simulation.PLANTS_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`PLANTS_INDEXES`
PLANTS_T_INDEXES = cnmetabolism_simulation.Simulation.PLANTS_T_INDEXES
#: plants post-processing variables
PLANTS_POSTPROCESSING_VARIABLES = []
#: concatenation of :attr:`PLANTS_T_INDEXES`, :attr:`PLANTS_RUN_VARIABLES <cnmetabolism.simulation.Simulation.PLANTS_RUN_VARIABLES>` and :attr:`PLANTS_POSTPROCESSING_VARIABLES`
PLANTS_RUN_POSTPROCESSING_VARIABLES = set(PLANTS_T_INDEXES + cnmetabolism_simulation.Simulation.PLANTS_RUN_VARIABLES + PLANTS_POSTPROCESSING_VARIABLES)

#: the indexes to locate the axes in the modelled system
AXES_INDEXES = cnmetabolism_simulation.Simulation.AXES_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`AXES_INDEXES`
AXES_T_INDEXES = cnmetabolism_simulation.Simulation.AXES_T_INDEXES
...
AXES_RUN_POSTPROCESSING_VARIABLES = set(AXES_T_INDEXES + cnmetabolism_simulation.Simulation.AXES_RUN_VARIABLES + AXES_POSTPROCESSING_VARIABLES)

#: the indexes to locate the phytomers in the modelled system
PHYTOMERS_INDEXES = cnmetabolism_simulation.Simulation.PHYTOMERS_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`PHYTOMERS_INDEXES`
PHYTOMERS_T_INDEXES = cnmetabolism_simulation.Simulation.PHYTOMERS_T_INDEXES
...
PHYTOMERS_RUN_POSTPROCESSING_VARIABLES = set(PHYTOMERS_T_INDEXES + cnmetabolism_simulation.Simulation.PHYTOMERS_RUN_VARIABLES + PHYTOMERS_POSTPROCESSING_VARIABLES)

#: the indexes to locate the organs in the modelled system
ORGANS_INDEXES = cnmetabolism_simulation.Simulation.ORGANS_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`ORGANS_INDEXES`
ORGANS_T_INDEXES = cnmetabolism_simulation.Simulation.ORGANS_T_INDEXES
...
ORGANS_RUN_POSTPROCESSING_VARIABLES = set(ORGANS_T_INDEXES + cnmetabolism_simulation.Simulation.ORGANS_RUN_VARIABLES + ORGANS_POSTPROCESSING_VARIABLES + ORGANS_RUN_VARIABLES_ADDITIONAL)

#: the indexes to locate the hidden zones in the modelled system
HIDDENZONE_INDEXES = cnmetabolism_simulation.Simulation.HIDDENZONE_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`HIDDENZONE_INDEXES`
HIDDENZONE_T_INDEXES = cnmetabolism_simulation.Simulation.HIDDENZONE_T_INDEXES
...
#: concatenation of :attr:`HIDDENZONE_T_INDEXES`, :attr:`HIDDENZONE_RUN_VARIABLES <cnmetabolism.simulation.Simulation.HIDDENZONE_RUN_VARIABLES>` and :attr:`HIDDENZONE_POSTPROCESSING_VARIABLES`
HIDDENZONE_RUN_POSTPROCESSING_VARIABLES = set(HIDDENZONE_T_INDEXES + cnmetabolism_simulation.Simulation.HIDDENZONE_RUN_VARIABLES + HIDDENZONE_RUN_VARIABLES_ADDITIONAL + HIDDENZONE_POSTPROCESSING_VARIABLES)

#: the indexes to locate the elements in the modelled system
ELEMENTS_INDEXES = cnmetabolism_simulation.Simulation.ELEMENTS_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`ELEMENTS_INDEXES`
ELEMENTS_T_INDEXES = cnmetabolism_simulation.Simulation.ELEMENTS_T_INDEXES
...
#: concatenation of :attr:`ELEMENTS_T_INDEXES`, :attr:`ELEMENTS_RUN_VARIABLES <cnmetabolism.simulation.Simulation.ELEMENTS_RUN_VARIABLES>` and :attr:`ELEMENTS_POSTPROCESSING_VARIABLES`
ELEMENTS_RUN_POSTPROCESSING_VARIABLES = set(ELEMENTS_T_INDEXES + cnmetabolism_simulation.Simulation.ELEMENTS_RUN_VARIABLES + ELEMENTS_RUN_VARIABLES_ADDITIONAL + ELEMENTS_POSTPROCESSING_VARIABLES)

#: the indexes to locate the soils in the modelled system
SOILS_INDEXES = cnmetabolism_simulation.Simulation.SOILS_INDEXES
#: concatenation of :attr:`T_INDEX` and :attr:`SOILS_INDEXES`
SOILS_T_INDEXES = cnmetabolism_simulation.Simulation.SOILS_T_INDEXES
...
SOILS_RUN_POSTPROCESSING_VARIABLES = SOILS_T_INDEXES + cnmetabolism_simulation.Simulation.SOILS_RUN_VARIABLES + SOILS_POSTPROCESSING_VARIABLES
...
class Roots:
    ...
    def calculate_WSC_g(sucrose):
        ...
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)

        :return: Water Soluble Carbohydrates (g)
        :rtype: float
        """
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS

        WSC_g = (sucrose * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
    ...
    def calculate_Conc_Nitrates(nitrates, mstruct):
        ...
        :param float or pandas.Series nitrates: Amount of nitrates (µmol` N)
        :param float or pandas.Series mstruct: Structural mass (g)
    ...
    def calculate_Conc_Amino_Acids(amino_acids, mstruct):
        ...
        :param float or pandas.Series amino_acids: Amount of amino acids (µmol` N)
        :param float or pandas.Series mstruct: Structural mass (g)

        :return: Amino_acid concentration (µmol` amino_acids g-1 mstruct)
        :rtype: float
        """
        return (amino_acids / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_N_RATIO) / mstruct
    ...
    def calculate_conc_sucrose(sucrose, mstruct):
        ...
        :rtype: float
        """
        return (sucrose / mstruct) / cnmetabolism_model.EcophysiologicalConstants.NB_C_SUCROSE

    @staticmethod
    def calculate_conc_cytokinins(cytokinins, mstruct):
        """Cytokinin concentration.

        :param float or pandas.Series cytokinins: Amount of cytokinins (AU)
        :param float or pandas.Series mstruct: Structural mass (g)
...
class Phloem:
    ...
    def calculate_conc_amino_acids(amino_acids, mstruct_axis):
        ...
        :param float or pandas.Series amino_acids: Amount of amino_acids in phloem (µmol` N)
        :param float or pandas.Series mstruct_axis: The structural dry mass of the axis (g)

        :return: Amino_acids concentration (µmol` amino_acids g-1 mstruct)
        :rtype: float
        """
        return (amino_acids / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_N_RATIO) / mstruct_axis

    @staticmethod
    def calculate_conc_sucrose(sucrose, mstruct_axis):
        """Sucrose concentration. Related to the structural dry mass of the culm

        :param float or pandas.Series sucrose: Amount of sucrose in phloem (µmol` C)
        :param float or pandas.Series mstruct_axis: The structural dry mass of the axis (g)

        :return: Sucrose concentration (µmol` sucrose g-1 mstruct)
        :rtype: float
        """
        return (sucrose / cnmetabolism_model.EcophysiologicalConstants.NB_C_SUCROSE) / mstruct_axis
...
class Grains:
    ...
    def calculate_dry_mass(structure, starch, proteins):
        ...
        :param float or pandas.Series structure: Grain structural C mass (µmol` C)
        :param float or pandas.Series starch:  Grain starch content (µmol` C)
        :param float or pandas.Series proteins: Grain protein content (µmol` N)
        ...
        C_mass = ((structure + starch) * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO

        #: N mass, grain proteins were supposed to be gluten mainly composed of Glu, Gln and Pro
        N_mass = (proteins * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) / cnmetabolism_model.Grains.AMINO_ACIDS_MOLAR_MASS_N_RATIO
    ...
    def calculate_WSC_g(sucrose, starch):
        ...
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)
        :param float or pandas.Series starch: Amount of sucrose (µmol` C)
        ...
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        WSC_g = ((sucrose * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                 (starch * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO)
    ...
    def calculate_protein_N_mass(proteins):
        ...
        :rtype: float
        """
        mass_N_proteins = proteins * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS  #: Mass of nitrogen in proteins (g)
...
class Endosperm:
    ...
    def calculate_dry_mass(starch, proteins, mstruct):
        ...
        :param float or pandas.Series starch:  Endosperm starch content (µmol` C)
        :param float or pandas.Series proteins: Endosperm protein content (µmol` N)
        :param float or pandas.Series mstruct: structural mass (g)
        ...
        """
        #: Carbohydrates mass, grain carbohydrates supposed to be mainly starch i.e. glucose polymers (C6 H12 O6)
        C_mass = (starch * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
    ...
    def calculate_starch_g(starch):
        ...
        """
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        starch_g = (starch * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
...
class HiddenZone:
    ...
    def calculate_dry_mass(sucrose, fructan, amino_acids, proteins, mstruct):
        ...
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)
        :param float or pandas.Series fructan: Amount of fructan (µmol` C)
        :param float or pandas.Series amino_acids: Amount of amino acids (µmol` N)
        :param float or pandas.Series proteins: Amount of proteins (µmol` N)
        :param float or pandas.Series mstruct: structural mass (g)
        ...
        """
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        N_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS
        dry_mass = ((sucrose * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                    (fructan * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                    (amino_acids * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    (proteins * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    mstruct)
    ...
    def calculate_C_g(sucrose, starch, fructan, amino_acids, proteins, mstruct):
        ...
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)
        :param float or pandas.Series starch: Amount of starch (µmol` C)
        :param float or pandas.Series fructan: Amount of fructan (µmol` C)
        :param float or pandas.Series amino_acids: Amount of amino acids (µmol` N)
        :param float or pandas.Series proteins: Amount of proteins (µmol` N)
        :param float or pandas.Series mstruct: structural mass (g)
        ...
        C_mass = ((sucrose * 1E-6 * C_MOLAR_MASS) +
                  ...
                  cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_C_RATIO / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                  (proteins * 1E-6 * N_MOLAR_MASS) * cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_C_RATIO / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                  mstruct * cnmetabolism_model.EcophysiologicalConstants.RATIO_C_mstruct)
    ...
    def calculate_N_g(amino_acids, proteins, Nstruct):
        ...
        :param float or pandas.Series amino_acids: Amount of amino acids (µmol` N)
        :param float or pandas.Series proteins: Amount of proteins (µmol` N)
        :param float or pandas.Series Nstruct: N structural mass(g)
    ...
    def calculate_fructan_g(fructan):
        ...
        :param float or pandas.Series fructan: Amount of fructans (µmol` C)

        :return: Dry mass (g)
        :rtype: float
        """
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        return (fructan * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
    ...
    def calculate_proteins_g(proteins):
        ...
        """
        N_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS
        return (proteins * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.PROTEINS_MOLAR_MASS_N_RATIO

    @staticmethod
    def calculate_Conc_Amino_Acids(amino_acids, mstruct):
        """Amino acid concentration.

        :param float or pandas.Series amino_acids: N amino acids (µmol` N)
        :param float or pandas.Series mstruct: Structural mass
    ...
    def calculate_conc_sucrose(sucrose, mstruct):
        ...
        :param float or pandas.Series sucrose: C sucrose (µmol` C)
        :param float or pandas.Series mstruct: Structural mass
    ...
    def calculate_conc_fructan(fructan, mstruct):
        ...
        :param float or pandas.Series fructan: C fructan (µmol` C)
        :param float or pandas.Series mstruct: Structural mass

        :return: Fructan concentration (µmol` fructan g-1 mstruct, eq. glucose).
        :rtype: float
        """
        return (fructan / mstruct) / cnmetabolism_model.EcophysiologicalConstants.NB_C_HEXOSES

    @staticmethod
    def calculate_conc_protein(proteins, mstruct):
        """Proteins concentration.

        :param float or pandas.Series proteins: N proteins (µmol` N)
        :param float or pandas.Series mstruct: Structural mass

        :return: Protein concentration (g proteins g-1 mstruct)
        :rtype: float
        """
        mass_N_proteins = proteins * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS  #: Mass of N in proteins (g)
        masS_proteins = mass_N_proteins / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO  #: Total mass of proteins (g)
    ...
    def calculate_RER(delta_leaf_L, leaf_L, delta_t):
        ...
        :param float or pandas.Series delta_leaf_L: delta of leaf length between t and t-1 (m)
        :param float or pandas.Series leaf_L: leaf length (m)
        :param float or pandas.Series delta_t: delta_t (s)
...
class Element:
    ...
    def calculate_dry_mass(triosesP, sucrose, starch, fructan, nitrates, amino_acids, proteins, mstruct):
        ...
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)
        :param float or pandas.Series starch: Amount of sucrose (µmol` C)
        :param float or pandas.Series fructan: Amount of sucrose (µmol` C)
        :param float or pandas.Series nitrates: Amount of nitrates (µmol` N)
        :param float or pandas.Series amino_acids: Amount of sucrose (µmol` N)
        :param float or pandas.Series proteins: Amount of sucrose (µmol` N)
        :param float or pandas.Series mstruct: structural mass (g)
        ...
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        N_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS
        dry_mass = ((triosesP * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.TRIOSESP_MOLAR_MASS_C_RATIO +
                    (sucrose * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                    (starch * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                    (fructan * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                    (nitrates * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.NITRATES_MOLAR_MASS_N_RATIO +
                    (amino_acids * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    (proteins * 1E-6 * N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    mstruct)
    ...
    def calculate_WSC_g(triosesP, sucrose, starch, fructan):
        ...
        :param float or pandas.Series triosesP: Amount of triose phosphates (µmol` C)
        :param float or pandas.Series sucrose: Amount of sucrose (µmol` C)
        :param float or pandas.Series starch: Amount of sucrose (µmol` C)
        :param float or pandas.Series fructan: Amount of sucrose (µmol` C)
        ...
        """
        C_MOLAR_MASS = cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS
        WSC_g = ((triosesP * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.TRIOSESP_MOLAR_MASS_C_RATIO +
                 (sucrose * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                 (starch * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                 (fructan * 1E-6 * C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO
                 )
    ...
    def calculate_conc_triosesP(triosesP, mstruct):
        ...
        :rtype: float
        """
        return (triosesP / mstruct) / cnmetabolism_model.EcophysiologicalConstants.NB_C_TRIOSEP
    ...
    def calculate_conc_starch(starch, mstruct):
        ...
        :rtype: float
        """
        return (starch / mstruct) / cnmetabolism_model.EcophysiologicalConstants.NB_C_HEXOSES
    ...
    def calculate_SLN(nitrates, amino_acids, proteins, Nstruct, green_area):
        ...
        :param float or pandas.Series  nitrates: Amount of nitrates (µmol` N)
        :param float or pandas.Series  amino_acids: Amount of amino_acids (µmol` N)
        :param float or pandas.Series  proteins: Amount of proteins (µmol` N)
        :param float or pandas.Series  Nstruct: Structural N (g)
        :param float or pandas.Series  green_area: Green area (m-2)

        :return: Surfacic Leaf Nitrogen (SLN, g.m-2)
        :rtype: float
        """
        mass_N_tot = (nitrates + amino_acids + proteins) * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS + Nstruct
    ...
    def calculate_SLN_nonstruct(nitrates, amino_acids, proteins, green_area):
        ...
        :rtype: float
        """
        mass_N_tot = (nitrates + amino_acids + proteins) * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS
        return mass_N_tot / green_area
    ...
    def calculate_SLA(dry_mass, green_area):
        ...
        :param float or pandas.Series  dry_mass: Dry mass (g)
        :param float or pandas.Series  green_area: Green area (m-2)
...
# PLEASE USE THIS FUNCTION TO APPLY POST-PROCESSING ON THE OUTPUT OF CN-METABOLISM -
# -----------------------------------------------------------------------------
...
def postprocessing(plants_df=None, axes_df=None, metamers_df=None, hiddenzones_df=None, organs_df=None, elements_df=None, soils_df=None, delta_t=1):
    """
    Compute post-processing from CN-Metabolism outputs, and format the post-processing to :class:`dataframes <pandas.DataFrame>`.
    ...
    :param pandas.DataFrame plants_df: CN-Metabolism outputs at plant scale (see :attr:`simulation.Simulation.PLANTS_RUN_VARIABLES`)
    :param pandas.DataFrame axes_df: CN-Metabolism outputs at axis scale (see :attr:`simulation.Simulation.AXES_RUN_VARIABLES`)
    :param pandas.DataFrame metamers_df: CN-Metabolism outputs at phytomer scale (see :attr:`simulation.Simulation.PHYTOMERS_RUN_VARIABLES`)
    :param pandas.DataFrame hiddenzones_df: CN-Metabolism outputs at hidden zone scale (see :attr:`simulation.Simulation.HIDDENZONE_RUN_VARIABLES`)
    :param pandas.DataFrame organs_df: CN-Metabolism outputs at organ scale (see :attr:`simulation.Simulation.ORGANS_RUN_VARIABLES`)
    :param pandas.DataFrame elements_df: CN-Metabolism outputs at element scale (see :attr:`simulation.Simulation.ELEMENTS_RUN_VARIABLES`)
    :param pandas.DataFrame soils_df: CN-Metabolism outputs at soil scale (see :attr:`simulation.Simulation.SOILS_RUN_VARIABLES`)
    ...
    if plants_df is not None:
        ...
        pp_plants_df = pp_plants_df.reindex(list(PLANTS_RUN_POSTPROCESSING_VARIABLES), axis=1, copy=False)
    ...
    if metamers_df is not None:
        ...
        pp_metamers_df = pp_metamers_df.reindex(list(PHYTOMERS_RUN_POSTPROCESSING_VARIABLES), axis=1, copy=False)
    ...
    if organs_df is not None and axes_df is not None:
        ...
        organs_df['sum_dry_mass'] = (((organs_df.fillna(0)['structure'] + organs_df.fillna(0)[
            'starch']) * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO) +
                                     (organs_df.fillna(0)[
                                          'sucrose'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                                     (organs_df.fillna(0)['starch'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.HEXOSE_MOLAR_MASS_C_RATIO +
                                     (organs_df.fillna(0)[
                                          'nitrates'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.NITRATES_MOLAR_MASS_N_RATIO +
                                     (organs_df.fillna(0)[
                                          'amino_acids'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                                     (organs_df.fillna(0)[
                                          'proteins'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                                     organs_df.fillna(0)['mstruct'])
        organs_df['C_g'] = ((organs_df.fillna(0)['sucrose'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) +
                            (organs_df.fillna(0)['starch'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.C_MOLAR_MASS) +
                            (organs_df.fillna(0)[
                                 'amino_acids'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) * cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_C_RATIO /
                            cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO + (organs_df.fillna(0)['proteins'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) *
                            cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_C_RATIO / cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                            organs_df.fillna(0)['mstruct'] * cnmetabolism_model.EcophysiologicalConstants.RATIO_C_mstruct)
        organs_df['N_g'] = ((organs_df.fillna(0)['nitrates'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) +
                            (organs_df.fillna(0)['amino_acids'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) +
                            (organs_df.fillna(0)['proteins'] * 1E-6 * cnmetabolism_model.EcophysiologicalConstants.N_MOLAR_MASS) +
        ...
        pp_organs_df.loc[pp_organs_df.organ == 'roots', 'Conc_cytokinins'] = Roots.calculate_conc_cytokinins(roots_df['cytokinins'], roots_df['mstruct'])
        R_residual = np.array(
            list(map(respiration_model.RespirationModel.R_residual, roots_df['sucrose'], roots_df['mstruct'] * cnmetabolism_model.Roots.PARAMETERS.ALPHA, roots_df['Total_Organic_Nitrogen'],
                     soils_df['Tsoil'])))
        ...
        pp_organs_df = pp_organs_df.reindex(columns=list(ORGANS_RUN_POSTPROCESSING_VARIABLES), copy=False)
    ...
    if elements_df is not None:
        ...
        for organ_type, parameters_class in \
                (('ear', cnmetabolism_parameters.CHAFF_ELEMENT_PARAMETERS),
                 ('blade', cnmetabolism_parameters.LAMINA_ELEMENT_PARAMETERS),
                 ('internode', cnmetabolism_parameters.INTERNODE_ELEMENT_PARAMETERS),
                 ('peduncle', cnmetabolism_parameters.PEDUNCLE_ELEMENT_PARAMETERS),
                 ('sheath', cnmetabolism_parameters.SHEATH_ELEMENT_PARAMETERS)):
            if organ_type not in grouped.groups:
                continue
            group = grouped.get_group(organ_type)
            if len(group) == 0:  # TODO: faire mm tri que dans simulation de cnmetabolism (surface nulle)
                continue
            curr_organ_elements_df = elements_df.loc[group.index]
            pp_curr_organ_elements_df = pp_elements_df.loc[group.index]
            R_residual = np.vectorize(respiration_model.RespirationModel.R_residual)(curr_organ_elements_df['sucrose'].fillna(0), curr_organ_elements_df['mstruct'].fillna(0) * parameters_class.ALPHA,
                                                                                    curr_organ_elements_df['Total_Organic_Nitrogen'].fillna(0), curr_organ_elements_df['Ts'].fillna(0))
            pp_curr_organ_elements_df.loc[:, 'R_residual'] = R_residual
        pp_elements_df = pp_elements_df.reindex(columns=list(ELEMENTS_RUN_POSTPROCESSING_VARIABLES))
    ...
    if hiddenzones_df is not None:
        ...
        if set(hiddenzones_df.columns).issuperset(['delta_leaf_L', 'leaf_L']):
            # this is temporary: those post-processing should be done in model "morphogenesis"
            pp_hiddenzones_df.loc[:, 'RER'] = HiddenZone.calculate_RER(hiddenzones_df['delta_leaf_L'], hiddenzones_df['leaf_L'], delta_t)
        pp_hiddenzones_df = pp_hiddenzones_df.reindex(columns=list(HIDDENZONE_RUN_POSTPROCESSING_VARIABLES), copy=False)
        ...
        if (hiddenzones_df is not None) and (organs_df is not None) and (elements_df is not None):
            ...
            if not elements_df.empty:
                ...
                if sum_mstruct_laminae.empty:
                    ...
                    mux = (pd.MultiIndex.from_product((range(sum_mstruct_shoot.index.get_level_values(0).min(), sum_mstruct_laminae.index.get_level_values(0).max() + 1),
            ...
            # C non structural
            sum_NSC_g = sum_C_g.values - (axes_df['mstruct'] * cnmetabolism_model.EcophysiologicalConstants.RATIO_C_mstruct)
            ...
            if not elements_df.empty:
                ...
                mux = (pd.MultiIndex.from_product((range(pp_axes_df.index.get_level_values(0).min(), tillers_photosynthesis.index.get_level_values(0).max() + 1),
            ...
            # C exuded
            roots_df['sum_C_exuded'] = (roots_df.C_exudation + roots_df.N_exudation * cnmetabolism_model.EcophysiologicalConstants.AMINO_ACIDS_C_RATIO / cnmetabolism_model.
                                          EcophysiologicalConstants.AMINO_ACIDS_N_RATIO) * roots_df.mstruct
            C_exuded_roots = roots_df['sum_C_exuded'].reset_index(drop=True)
            ...
            if sum_dry_mass_laminae.sum() != 0:
                pp_axes_df.loc[:, 'Cont_WSC_DM_laminae'] = sum_WSC_g_laminae.to_numpy() / sum_dry_mass_laminae.replace(0, np.nan).to_numpy() * 100
            else:
            ...
            pp_axes_df.loc[:, 'C_exuded'] = C_exuded_roots.values

        pp_axes_df = pp_axes_df.reindex(list(AXES_RUN_POSTPROCESSING_VARIABLES), axis=1)
...
def generate_graphs(axes_df=None, hiddenzones_df=None, organs_df=None, elements_df=None, soils_df=None, meteo_data=None, graphs_dirpath='.'):
    ...
    :param pandas.DataFrame axes_df: CN-Metabolism outputs and post-processing at axis scale (see :attr:`PLANTS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame hiddenzones_df: CN-Metabolism outputs at hidden zone scale (see :attr:`HIDDENZONE_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame organs_df: CN-Metabolism outputs at organ scale (see :attr:`ORGANS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame elements_df: CN-Metabolism outputs at element scale (see :attr:`ELEMENTS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame soils_df: CN-Metabolism outputs at soil scale (see :attr:`SOILS_RUN_POSTPROCESSING_VARIABLES`)
    ...
    if elements_df is not None:
        ...
        for org_ph in (['blade'], ['sheath'], ['internode'], ['peduncle', 'ear']):
            for variable_name, variable_label in graph_variables_ph_elements.items():
                ...
                integration_tools.plot_outputs(elements_df,
                                                  x_name=x_name,
    ...
    if organs_df is not None:
        ...
        for org in (['roots'], ['grains'], ['phloem'], ['endosperm']):
            for variable_name, variable_label in graph_variables_organs.items():
                ...
                integration_tools.plot_outputs(organs_df,
                                                  x_name=x_name,
                                                  y_name=variable_name,
                                                  x_label=x_label,
                                                  y_label=variable_label,
    ...
    if hiddenzones_df is not None:
        ...
        for variable_name, variable_label in graph_variables_hiddenzones.items():
            ...
            integration_tools.plot_outputs(hiddenzones_df,
                                              x_name=x_name,
                                              y_name=variable_name,
                                              x_label=x_label,
                                              y_label=variable_label,
    ...
    graph_variables_axes = {'mstruct': 'Axis mstruct (g)',
                            ...
                            'N_content_total_DM_shoot': u'N content in the shoot accounting for partly senesced leaves (% DM)',
    ...
    for variable_name, variable_label in graph_variables_axes.items():
        ...
        integration_tools.plot_outputs(axes_df,
                                          x_name=x_name,
                                          y_name=variable_name,
                                          x_label=x_label,
                                          y_label=variable_label,
