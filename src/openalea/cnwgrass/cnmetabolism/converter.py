...
from openalea.cnwgrass.cnmetabolism import model, simulation

"""
    cnmetabolism.converter
    ~~~~~~~~~~~~~~~~~

    The module :mod:`cnmetabolism.converter` defines functions to convert
    :class:`dataframes <pandas.DataFrame>` to/from CN-Metabolism inputs or outputs format.
...
CNMETABOLISM_CLASSES_TO_DATAFRAME_ORGANS_MAPPING = {model.Internode: 'internode', model.Lamina: 'blade',
...
DATAFRAME_TO_CNMETABOLISM_ELEMENTS_NAMES_MAPPING = {'HiddenElement': 'enclosed_element', 'StemElement': 'exposed_element', 'LeafElement1': 'exposed_element'}
...
def from_dataframes(axes_inputs=None, organs_inputs=None, hiddenzones_inputs=None, elements_inputs=None, soils_inputs=None, update_parameters=None):
    ...
    :param pandas.DataFrame or None soils_inputs: Soils inputs, with one line by soil.
        ...
        for plant_index in organs_inputs.plant.unique():
            ...
            for axis_label in curr_axes_labels:
                ...
                for axis_attribute_name, axis_attribute_class in (('roots', model.Roots), ('phloem', model.Phloem), ('grains', model.Grains), ('endosperm', model.Endosperm)):
                    organ_label = CNMETABOLISM_CLASSES_TO_DATAFRAME_ORGANS_MAPPING[axis_attribute_class]
                ...
                for metamer_index in curr_metamers_indexes:
                    ...
                    for phytomer_attribute_name, phytomer_attribute_class, phytomer_attribute_element_class in \
                        ...
                        organ_label = CNMETABOLISM_CLASSES_TO_DATAFRAME_ORGANS_MAPPING[phytomer_attribute_class]
                        ...
                        if metamer_index in curr_metamers_indexes_for_elements:
                            ...
                            for mtg_element_label, cnmetabolism_element_name in DATAFRAME_TO_CNMETABOLISM_ELEMENTS_NAMES_MAPPING.items():
                                ...
                                setattr(organ, cnmetabolism_element_name, element)
                    ...
                    if metamer_index in curr_metamers_indexes_for_hiddenzones:
                        ...
                        hiddenzone = model.HiddenZone(CNMETABOLISM_CLASSES_TO_DATAFRAME_ORGANS_MAPPING[
                                                          model.HiddenZone], **hiddenzone_dict)
...
def to_dataframes(population=None, soils=None):
    """ Convert a CN-Metabolism :class:`population <model.Population>` and/or a dictionary of :class:`soils <model.Soil>` to Pandas dataframes.

    If `population` is not None, convert `population` to Pandas dataframes.
    If `soils` is not None, convert `soils` to Pandas dataframe.

    :param model.Population population: The CN-Metabolism population to convert.
