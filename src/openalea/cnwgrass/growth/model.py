...
from openalea.cnwgrass.growth import parameters as growth_parameters

"""
    growth.model
    ~~~~~~~~~~~~~

    The module :mod:`growth.model` defines the equations of the kinetic of leaf growth (mass flows) according to leaf elongation. Also includes root growth.

"""

class GrowthModel(object):
    def __init__(self):
        self.parameters = growth_parameters.parameters_factory()
        self.organ_init = growth_parameters.OrganInit()

        #: the inputs needed by Growth
        # -: hiddenzones
    ...
    def calculate_ratio_mstruct_DM(mstruct, sucrose, fructans, amino_acids, proteins):
        ...
        :rtype: float
        """
        C_MOLAR_MASS = growth_parameters.C_MOLAR_MASS
        N_MOLAR_MASS = growth_parameters.N_MOLAR_MASS
        dry_mass = ((sucrose * 1E-6 * C_MOLAR_MASS) / growth_parameters.HEXOSE_MOLAR_MASS_C_RATIO +
                    (fructans * 1E-6 * C_MOLAR_MASS) / growth_parameters.HEXOSE_MOLAR_MASS_C_RATIO +
                    (amino_acids * 1E-6 * N_MOLAR_MASS) / growth_parameters.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    (proteins * 1E-6 * N_MOLAR_MASS) / growth_parameters.AMINO_ACIDS_MOLAR_MASS_N_RATIO +
                    mstruct)
    ...
    def calculate_delta_leaf_enclosed_mstruct(self, leaf_L, delta_leaf_L, ratio_mstruct_DM, init_leaf_L=None, leaf_pseudo_age=None):
        ...
        :param float init_leaf_L: Total leaf length before update in Hydraulics sub-model (m). Not used in this instance of the model
    ...
    def calculate_s_Nstruct_amino_acids(delta_hiddenzone_Nstruct, delta_lamina_Nstruct, delta_sheath_Nstruct, delta_internode_Nstruct):
        ...
        :rtype: float
        """
        return (delta_hiddenzone_Nstruct + delta_lamina_Nstruct + delta_sheath_Nstruct + delta_internode_Nstruct) / growth_parameters.N_MOLAR_MASS * 1E6
    ...
    def calculate_s_mstruct_sucrose(self, delta_hiddenzone_mstruct, delta_lamina_mstruct, delta_sheath_mstruct, s_Nstruct_amino_acids_N):
        ...
        s_mstruct_C = (delta_hiddenzone_mstruct + delta_lamina_mstruct + delta_sheath_mstruct) * self.parameters.RATIO_SUCROSE_MSTRUCT / growth_parameters.C_MOLAR_MASS * 1E6  #: Total C for mstruct growth (µmol C)
    ...
    def calculate_roots_mstruct_growth(self, sucrose, amino_acids, mstruct, delta_teq, postflowering_stages, nb_leaves,
        ...
        if conc_sucrose_effective > 0.:
            ...
            mstruct_C_growth = 0.
        mstruct_growth = mstruct_C_growth * growth_parameters.CONVERSION_MMOL_C_G_MSTRUCT_ROOTS  #: root growth (g of structural dry mass)

        Nstruct_growth = mstruct_growth * growth_parameters.RATIO_N_MSTRUCT_ROOTS  #: root growth in N (g of structural dry mass)
        Nstruct_N_growth = min(amino_acids, (Nstruct_growth / growth_parameters.N_MOLAR_MASS) * 1E6)  #: root growth in nitrogen (µmol N)
    ...
    def calculate_roots_s_mstruct_sucrose(self, delta_roots_mstruct, s_Nstruct_amino_acids_N):
        ...
        s_Nstruct_amino_acids = s_Nstruct_amino_acids_N / self.parameters.AMINO_ACIDS_N_RATIO  #: µmol of AA
        s_mstruct_amino_acids_C = s_Nstruct_amino_acids * self.parameters.AMINO_ACIDS_C_RATIO  #: µmol of C coming from AA
        s_mstruct_C = delta_roots_mstruct * self.parameters.RATIO_SUCROSE_MSTRUCT / growth_parameters.C_MOLAR_MASS * 1E6  #: Total C used for mstruct growth (µmol C)
...
class GrowthModelHydraulics(GrowthModel):
    def __init__(self):
        super().__init__()
        self.parameters = growth_parameters.parameters_factory(hydraulics=True)
    ...
    def calculate_delta_leaf_enclosed_mstruct(self, leaf_L, delta_leaf_L, ratio_mstruct_DM, init_leaf_L=0, leaf_pseudo_age=0):
        ...
        :param float leaf_L: Total leaf length after update in Hydraulics sub-model (m)
        :param float delta_leaf_L: delta of leaf length (m)
        :param float ratio_mstruct_DM: Ratio mstruct/dry matter (dimensionless)
        :param float init_leaf_L: Total leaf length before update in Hydraulics sub-model (m)
        :param float leaf_pseudo_age: Pseudo age of the leaf since beginning of automate elongation (s)
    ...
    def calculate_roots_mstruct_growth(self, sucrose, amino_acids, mstruct, delta_teq, postflowering_stages,
        ...
        mstruct_growth = mstruct_C_growth * growth_parameters.CONVERSION_MMOL_C_G_MSTRUCT_ROOTS * hydraulic_regulation  #: root growth (g of structural dry mass)

        Nstruct_growth = mstruct_growth * growth_parameters.RATIO_N_MSTRUCT_ROOTS  #: root growth in N (g of structural dry mass)
        Nstruct_N_growth = min(amino_acids,
                               (Nstruct_growth / growth_parameters.N_MOLAR_MASS) * 1E6)  #: root growth in nitrogen (µmol N)
