...
"""
    senescence.converter
    ~~~~~~~~~~~~~~~~~~~~~

    The module :mod:`senescence.converter` defines functions to convert
    :class:`dataframes <pandas.DataFrame>` to/from Senescence inputs or outputs format.

"""

#: the inputs needed by Senescence at roots scale
SENESCENCE_ROOTS_INPUTS = ['sucrose', 'amino_acids', 'mstruct', 'senesced_mstruct', 'Nstruct', 'cytokinins']

#: the inputs needed by Senescence at axis scale
SENESCENCE_AXES_INPUTS = ['delta_teq', 'delta_teq_roots', 'sum_TT']

#: the inputs needed by Senescence at element scale
SENESCENCE_ELEMENTS_INPUTS = ['green_area', 'senesced_length_element', 'length', 'proteins', 'mstruct', 'senesced_mstruct', 'max_proteins', 'Nstruct', 'nitrates', 'amino_acids', 'starch', 'fructan',
                               'cytokinins', 'sucrose', 'is_growing', 'max_mstruct', 'Nresidual', 'age']

#: the outputs computed by Senescence at roots scale
SENESCENCE_ROOTS_OUTPUTS = ['rate_mstruct_death', 'mstruct', 'senesced_mstruct', 'Nstruct', 'cytokinins']

#: the outputs computed by Senescence at axis scale
SENESCENCE_AXES_OUTPUTS = []

#: the outputs computed by Senescence at elements scale
SENESCENCE_ELEMENTS_OUTPUTS = ['senesced_length_element', 'green_area', 'mstruct', 'senesced_mstruct', 'Nstruct', 'starch', 'sucrose', 'fructan', 'proteins', 'amino_acids', 'cytokinins', 'nitrates',
                                'N_content_total', 'Nresidual', 'is_over']

#: the inputs and outputs of Senescence at roots scale
SENESCENCE_ROOTS_INPUTS_OUTPUTS = sorted(list(set(SENESCENCE_ROOTS_INPUTS + SENESCENCE_ROOTS_OUTPUTS)))

#: the inputs and outputs of Senescence at axis scale
SENESCENCE_AXES_INPUTS_OUTPUTS = sorted(list(set(SENESCENCE_AXES_INPUTS + SENESCENCE_AXES_OUTPUTS)))

#: the inputs and outputs of Senescence at elements scale
SENESCENCE_ELEMENTS_INPUTS_OUTPUTS = sorted(list(set(SENESCENCE_ELEMENTS_INPUTS + SENESCENCE_ELEMENTS_OUTPUTS)))
...
def from_dataframes(roots_inputs, axes_inputs, elements_inputs):
    """
    Convert inputs/outputs from Pandas dataframes to Senescence format.
    ...
    seealso:: see :attr:`simulation.Simulation.inputs` and :attr:`simulation.Simulation.outputs`
       for the structure of Senescence inputs/outputs.
...
def to_dataframes(data_dict):
    """
    Convert inputs/outputs from Senescence format to Pandas dataframe.

    :param dict data_dict: The inputs/outputs in Senescence format.
    ...
    """
    dataframes_dict = {}
    for (current_key, current_topology_columns, current_inputs_outputs_names) in (('roots', ROOTS_TOPOLOGY_COLUMNS, SENESCENCE_ROOTS_INPUTS_OUTPUTS),
                                                                                  ('axes', AXES_TOPOLOGY_COLUMNS, SENESCENCE_AXES_INPUTS_OUTPUTS),
                                                                                  ('elements', ELEMENTS_TOPOLOGY_COLUMNS, SENESCENCE_ELEMENTS_INPUTS_OUTPUTS)):
