...
def organ_temperature(w, z, Zh, Ur, PAR, gsw, Ta, Ts, RH, organ_name):
    ...
    Tr = max(0., (s * Rn + (parameters.RHOCP * VPDa) / (rbh + ra)) / (
                parameters.LAMBDA * (s + parameters.GAMMA * ((rbw + ra + rswp) / (rbh + ra)))))  #: mm s-1
...
def _inhibition_by_NSC(NSC):
    ...
    if NSC <= parameters.WSC_min:
        ...
        return min(parameters.Inhibition_max * (NSC - parameters.WSC_min) / (
                    parameters.K_Inhibition + NSC - parameters.WSC_min), 1)
