...
from openalea.hydraulics import simulation as hydraulics_simulation, parameters as hydraulics_parameters
...
class HiddenZone:
    ...
    @staticmethod
    def calculate_LER(leaf_L, init_leaf_L, delta_t):
        """Calculates Leaf elongation rate in phase II

        :param float leaf_L: Leaf length at time t (m)
        :param float init_leaf_L: Leaf length at time t-1 (m)
        :param float delta_t: Delta time (h)

        :return: Leaf elongation rate (mm h-1)
        :rtype: float
        """
        LER = ((leaf_L - init_leaf_L) * 1E03) / delta_t
    ...
    def calculate_conc_solutes_vol(sucrose, fructan, amino_acids, volume):
        ...
        :rtype: float
        """
        conc_solutes_vol = (sucrose / hydraulics_parameters.NB_C_SUCROSE + fructan / hydraulics_parameters.NB_C_SUCROSE + amino_acids / hydraulics_parameters.AMINO_ACIDS_N_RATIO) * 1E-06 / (volume * hydraulics_parameters.VSTORAGE)
    ...
    def calculate_conc_solutes_mass(fructan, sucrose, amino_acids, mstruct):
        ...
        :return: Solutes massic concentration (mol m-3)
        :rtype: float
        """
...
#           PLEASE USE THIS FUNCTION TO APPLY POST-PROCESSING ON THE OUTPUT OF HYDRAULICS     -
# ------------------------------------------------------------------------------------------------
...
def postprocessing(plants_df=None, axes_df=None, metamers_df=None, hiddenzones_df=None, organs_df=None, elements_df=None, soils_df=None, delta_t=1):
    """
    Compute post-processing from Hydraulics outputs, and format the post-processing to :class:`dataframes <pandas.DataFrame>`.
    ...
    :param pandas.DataFrame plants_df: Hydraulics outputs at plant scale (see :attr:`simulation.Simulation.PLANTS_RUN_VARIABLES`)
    :param pandas.DataFrame axes_df: Hydraulics outputs at axis scale (see :attr:`simulation.Simulation.AXES_RUN_VARIABLES`)
    :param pandas.DataFrame metamers_df: Hydraulics outputs at phytomer scale (see :attr:`simulation.Simulation.PHYTOMERS_RUN_VARIABLES`)
    :param pandas.DataFrame hiddenzones_df: Hydraulics outputs at hidden zone scale (see :attr:`simulation.Simulation.HIDDENZONE_RUN_VARIABLES`)
    :param pandas.DataFrame elements_df: Hydraulics outputs at element scale (see :attr:`simulation.Simulation.ELEMENTS_RUN_VARIABLES`)
    :param pandas.DataFrame organs_df: Hydraulics outputs at xylem scale (see :attr:`simulation.Simulation. ORGAN_RUN_VARIABLES`)
    :param pandas.DataFrame soils_df: Hydraulics outputs at soil scale (see :attr:`simulation.Simulation.SOILS_RUN_VARIABLES`)
    ...
    if axes_df is not None:
        ...
        pp_axes_df.loc[:, 'plant_WC_DM'] = axes_df['plant_water_content'] / (axes_df['plant_water_content'] + axes_df['mstruct'])
    ...
    if hiddenzones_df is not None:
        ...
        hiddenzones_df.loc[:, 'LER'] = HiddenZone.calculate_LER(hiddenzones_df['leaf_L'], hiddenzones_df['init_leaf_L'], delta_t)
    ...
    if elements_df is not None:
        ...
        for organ_type, parameters_class in (('blade', hydraulics_parameters.LAMINA_ELEMENT_PARAMETERS), ('internode', hydraulics_parameters.INTERNODE_ELEMENT_PARAMETERS), ('sheath', hydraulics_parameters.SHEATH_ELEMENT_PARAMETERS)):
...
def generate_graphs(axes_df=None, hiddenzones_df=None, organs_df=None, elements_df=None, soils_df=None, meteo_data=None, graphs_dirpath='.'):
    ...
    :param pandas.DataFrame axes_df: Hydraulics outputs and post-processing at axis scale (see :attr:`PLANTS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame hiddenzones_df: Hydraulics outputs at hidden zone scale (see :attr:`HIDDENZONE_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame elements_df: Hydraulics outputs at element scale (see :attr:`ELEMENTS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame organs_df: Hydraulics outputs at organ scale (see :attr:`ORGANS_RUN_POSTPROCESSING_VARIABLES`)
    :param pandas.DataFrame soils_df: Hydraulics outputs at soil scale (see :attr:`SOILS_RUN_POSTPROCESSING_VARIABLES`)
    ...
    if elements_df is not None:
        graph_variables_ph_elements = {'age': u'Age (°Cd)', 'organ_volume': 'Volume of organ based on dimensions (m3)', 'epsilon_volume': 'Volumetric elasticity (Mpa)', 'Total_Transpiration_turgor': u'Total transpiration of turgor model (g H2O)',
                                       'osmotic_water_potential': u'Osmotic water potential (MPa)', 'thickness': u'Thickness (m)', 'water_potential': u'Total water potential (MPa)',
                                       ...
                                       'resistance': u'Resistance (MPa s g$^{-1}$)', 'volume': u'Volume (m3)'}
    ...
    # 2) Hidden zones
    if hiddenzones_df is not None:
        graph_variables_hiddenzones = {'delta_hiddenzone_dimensions_plastic': u'dl / dt plastic (mm h-1)', 'LER': u'LER in phase II (mm h-1)', 'conc_solutes_vol': u'conc_solutes_vol', 'conc_solutes_mass': u'conc_solutes_mass',
                                       'length_hz_En': u'Length of HZ after En (m)', 'organ_volume': u'Volume of hz based on dimensions (m3)', 'phi_volume': u'Volumetric extensibility (MPa-1 h-1) ', 'epsilon_volume': u'Volumetric elasticity (Mpa)', 'leaf_pseudo_age': u'Leaf pseudo age (°Cd)',
                                        ...
                                        'length': u'Length of hz (m)', 'hiddenzone_age': u'Age (s)',
                                       'osmotic_water_potential': u'Osmotic water potential (MPa)', 'width': u'Width (m)', 'water_potential': u'Total water potential (MPa)',
                                       'turgor_water_potential': u'Turgor water potential (MPa)', 'water_content': u'Water content (g)', 'water_influx': u'Water flow Xylem (g)',
                                       'resistance': u'Resistance (MPa s g$^{-1}$)', 'thickness': u'Thickness (m)', 'volume': u'Volume (m3)', 'WC_mstruct': u'ratio WC_mstruct'}
    ...
    if organs_df is not None:
        graph_variables_organs = {'water_potential': u'Total water potential (MPa)'}
    ...
    # 4) Axes
    graph_variables_axes = {'Total_Transpiration_turgor': u'Total transpiration from turgor model (g H2O)', 'Growth': u'Growth (g H2O)',
                            'water_influx': u' Water flux from xylem to HZ and photosynthetic organs (g H2O)', 'plant_water_content': u'Total water content of the plant (g H2O)',
                            'plant_WC_DM': u'Ratio of plant water content per structural mass (%)'}
