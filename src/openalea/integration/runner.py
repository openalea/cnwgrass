...
from openalea.integration import build_outputs
...
def run(simulation_length, forced_start_time=0, run_simu=True, run_postprocessing=True, generate_graphs=True,
    ...
    if 'constant_Conc_Nitrates' in N_fertilizations.keys():
        cnmetabolism_facade_.soils[(1, 'MS')].constant_Conc_Nitrates = True  # TODO: make (1, 'MS') more general
        cnmetabolism_facade_.soils[(1, 'MS')].nitrates = N_fertilizations['constant_Conc_Nitrates'] * cnmetabolism_facade_.soils[(1, 'MS')].volume

    # Force root nitrate uptake if specified
    if external_soil_model and step_callback is not None:
        try:
            step_callback['nitrate_uptake'](0, cnmetabolism_facade_.population, g)
        except KeyError:
            print('Function name error in step_callback keys. It should be nitrate_uptake')

    # -- MODEL INTEGRATION --
    # Facade initialisation
    build_outputs_ = build_outputs.BuildOutputs(g, morphogenesis_facade_, growth_facade_, gasexchange_facade_, hydraulics_facade_)
    ...
    if run_simu:
        ...
        try:
            ...
            for t in range(START_TIME, SIMULATION_LENGTH, 1):
                print('t cnmetabolism is {}'.format(t))
                # if t == 1 or t >= 800:
                ...
                # run Gas-Exchange
                gasexchange_facade_.run(Ta, ambient_CO2, RH, Ur)

                # run Morphogenesis
                Tair, Tsoil = meteo.loc[t, ['air_temperature', 'soil_temperature']]
                morphogenesis_facade_.run(Tair, Tsoil, Zsowing)
                ...
                # run hydraulics
                if hydraulics and hydraulics_facade_ is not None:
                ...
                if t in N_fertilizations.keys():
                    cnmetabolism_facade_.soils[(1, 'MS')].nitrates += N_fertilizations[t]

                # Force root nitrate uptake if specified
                if external_soil_model and step_callback is not None:
                    try:
                        step_callback['nitrate_uptake'](t, cnmetabolism_facade_.population, g)
                    except KeyError:
                        print(
                            'Function name error in step_callback keys. It should be nitrate_uptake')
                # run CN-Metabolism
                cnmetabolism_facade_.run(Tair, Tsoil)

                # Adel 3D plant save
                # if t_cnmetabolism % 24 == 0:
                #     adel_wheat.scene(g).save(os.path.join(OUTPUTS_DIRPATH, 'ADEL', 't{}.bgeom'.format(t_cnmetabolism)))

                # append outputs at current step to global lists
                if (stored_times == 'all') or (t in stored_times):
                    axes_outputs, elements_outputs, hiddenzones_outputs, organs_outputs, soils_outputs = build_outputs_.build_outputs_df_from_MTG()
    ...
    if run_postprocessing:
        ...
        if not run_simu:
            ...
            delta_t = CNMETABOLISM_TIMESTEP * HOUR_TO_SECOND_CONVERSION_FACTOR

        # run the postprocessing
        postprocessing = cnmetabolism_facade.CNMetabolismFacade.postprocessing(axes_outputs_df=outputs_df_dict[AXES_OUTPUTS_FILENAME.split('.')[0]],
        ...
        if hydraulics:
            turgor_postprocessing = hydraulics_facade.hydraulicsFacade.postprocessing(axes_outputs_df=outputs_df_dict[AXES_OUTPUTS_FILENAME.split('.')[0]],
                                                                                          hiddenzone_outputs_df=outputs_df_dict[HIDDENZONES_OUTPUTS_FILENAME.split('.')[0]],
                                                                                          elements_outputs_df=outputs_df_dict[ELEMENTS_OUTPUTS_FILENAME.split('.')[0]],
                                                                                          organs_outputs_df=outputs_df_dict[ORGANS_OUTPUTS_FILENAME.split('.')[0]],
                                                                                          soils_outputs_df=outputs_df_dict[SOILS_OUTPUTS_FILENAME.split('.')[0]],
    ...
    if generate_graphs:
        ...
        integration_tools.additional_graphs(outputs_df_dict['axes_outputs'], outputs_df_dict['hiddenzones_outputs'], outputs_df_dict['elements_outputs'],
                                          ...
                                          plant_density[1], morphogenesis_facade_._simulation.model.parameters.RERmax.items(), GRAPHS_DIRPATH, data_obs)
